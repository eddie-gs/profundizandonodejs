...
app.put("/people/:index", (req, res) => {
   ...
   */
  if (people.filter((p)=> p.id === Number(req.params.index)).length > 0) { //Chequea si existe un elemento con esa ID
    posicionElem = people.map((p)=> p.id).indexOf(Number(req.params.index)) //Encontramos la posicion del elemento en el array
    nuevoElem = req.body
    nuevoElem.id = Number(req.params.index)
    antesdeElem = people.slice(0,posicionElem)
    people = [...antesdeElem,nuevoElem,...people.slice(posicionElem+1)]
    res.json(people)
  }else{
...
app.delete("/people/:index", (req, res) => {
   ...
   */
  //console.log(Number(req.params.index) !== people[2].id)
  let cantMatch = people.length
  people = people.filter((p)=>{return p.id !== Number(req.params.index)})
  console.log(cantMatch - people.length)
  res.json(people)
});
